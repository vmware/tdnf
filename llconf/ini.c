...
static
struct confline *unparse_ini_subsection(struct cnfnode *cn, struct confline *cl_list, unsigned int level)
{
	char buf[1024], ident[256];
	struct confline *cl;
...
int unparse_ini(struct cnfmodule *cm __attribute__((__unused__)), FILE *fptr, struct cnfnode *cn_root)
{
