...
TDNFGetPackagesWithSpecifiedType(
    Transaction* pTrans, PTDNF pTdnf, 
    ...
    PTDNF_PKG_INFO* pRemovePkgInfo)
{
    ...
    if(toRemove.count > 0)
    {
        pRemovePkgList = SolvCreatePackageList();
        if(!pRemovePkgList)
        {
            dwError = ERROR_TDNF_INVALID_PARAMETER;
            BAIL_ON_TDNF_ERROR(dwError);
    ...
    )
{
    ...
    switch(nAlterType)
    {
        ...
        case ALTER_DOWNGRADE:
            dwError = SolvAddPkgDowngradeJob(jobs, id);
            BAIL_ON_TDNF_ERROR(dwError);
            break;
        case ALTER_ERASE:
            dwError = SolvAddPkgEraseJob(jobs, id);
            BAIL_ON_TDNF_ERROR(dwError);
            break;
        case ALTER_REINSTALL:
        case ALTER_INSTALL:
            dwError = SolvAddPkgInstallJob(jobs, id);
            BAIL_ON_TDNF_ERROR(dwError);
            break;
        case ALTER_UPGRADE:
            pszPkg = SolvGetPkgNevrFromId(pTdnf->pSack, id);
        ...
        case ALTER_AUTOERASE:
            dwError = SolvAddPkgUserInstalledJob(jobs, id);
            BAIL_ON_TDNF_ERROR(dwError);
            break;
        default:
