    ...
    )
{
    ...
    for(nIndex = 0; nIndex < nPkgCount; ++nIndex)
    {
        dwError = TDNFAllocateString(ppszPkgNames[nIndex], &ppszCopyOfPkgNames[nIndex]);
        BAIL_ON_TDNF_ERROR(dwError);
    }
    ...
    if(ppszPkgNames)
    {
        while(*ppszPkgNames)
        {
            ...
            flags |= SELECTION_CANON|SELECTION_DOTARCH|SELECTION_REL;
                rflags = selection_make(pPool, &queueJob, *ppszPkgNames, flags);
    ...
    Solver** ppSolv)
{
    ...
    for (nJob = 0; nJob < queueJobs->count; nJob += 2)
    {
        ...
        if (dwMode == SOLVER_UPDATE && 
            pool_isemptyupdatejob(pQuery->pSack->pPool,
